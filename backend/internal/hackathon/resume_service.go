...
// 查询用户简历
func (hm *HackathonManager) GetResumeByAddr(ctx context.Context, userId primitive.ObjectID, nonAutoGen bool) Resume {
	...
	if !existed {
		...
		hm.Insert(ctx, userId, RESUME_DATA)
	}
...
// 更新用户简历
func (hm *HackathonManager) Insert(ctx context.Context, userId primitive.ObjectID, data string) primitive.ObjectID {
	resumeObj := &Resume{
		UserId: userId,
		Data:   data,
	}
